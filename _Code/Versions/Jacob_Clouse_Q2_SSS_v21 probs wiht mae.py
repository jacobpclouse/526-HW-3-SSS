...
if wantDownscale == False:
    ...
    # output_array = np.array(Image.open(reconstructName).convert('RGB'))
    output_array = np.array(Image.open(reconstructName).convert('RGB'))

    # convert the arrays to 1d arrays
    new_list = [[t[0] for t in inner_list] for inner_list in downscaled_array]
    # arr2_1d = output_array.flatten()
